<template>
    <div class="livewx-pane">
        ...
        <div class="form-check form-switch mb-3">
            ...
            >
            <label class="form-check-label" for="livewx-overlay">Radar overlay</label>
        </div>

        <label class="form-label mb-1" for="livewx-opacity">
        ...
        <label class="form-label mb-1" for="livewx-site-search">Radar site</label>
        <input
        ...
        <select
            ...
            @change="onSite"
        >
            <option :value="CONUS_SITE_ID">CONUS Mosaic</option>
            <optgroup
        ...
        <div class="form-check form-switch mb-3">
            ...
            >
            <label class="form-check-label" for="livewx-alerts">Watches &amp; warnings</label>
        </div>
        <p class="text-secondary small mb-3" v-if="state.alertsEnabled">
            {{ state.alertCount }} active nationwide (not limited to the selected radar).
        </p>
        <p class="small mb-3" v-if="state.selectedAlert">{{ state.selectedAlert }}</p>

        <h4 class="subheader">Replay</h4>
        <div class="d-flex gap-2 mb-2">
            <button class="btn btn-sm btn-outline-secondary" type="button" :disabled="!frames.length" @click="onPlay">
                {{ state.playing ? 'Pause' : 'Play' }}
            </button>
            <button class="btn btn-sm btn-outline-secondary" type="button" @click="onLive">Live</button>
        </div>
        <input
        ...
        <p class="small mb-0" :class="state.error ? 'text-danger' : 'text-secondary'">
